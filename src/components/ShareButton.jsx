...
export default function ShareButton({ text, reference }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        aria-label="Vers teilen"
        aria-expanded={open}
        className="flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-full border border-gray-200 bg-white text-gray-500 hover:border-forest-300 hover:text-forest-600 transition-all"
      >
        ↗ Teilen
      </button>
      {open && (
        <>
          ...
          <div className="absolute bottom-full left-0 mb-2 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-20 min-w-[160px]">
            <a
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-forest-50"
